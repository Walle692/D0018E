...
export async function searchSellerOrders({ search_type, search_object, limit = 10, offset = 0 }) {
  const res = await fetch(`${BASE_URL}/seller/orders?limit=${limit}&offset=${offset}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({ search_type, search_object }),
  ...
  const text = await res.text().catch(() => '')
  if (!res.ok) throw new Error(text || 'seller order search failed')
  return text ? JSON.parse(text) : []
}
