...
(
  (SELECT basket_id FROM myschema.basket b
   ...
   LIMIT 1),
  (SELECT product_id FROM myschema.products WHERE sku='SKU-ACME-24'),
  1,
  (SELECT price FROM myschema.products WHERE sku='SKU-ACME-24')
),
(
  (SELECT basket_id FROM myschema.basket b
   JOIN myschema.users u ON u.user_id=b.basket_user_id
   ...
   ORDER BY b.basket_id DESC
   LIMIT 1),
  (SELECT product_id FROM myschema.products WHERE sku='SKU-VB-22'),
  2,
  (SELECT price FROM myschema.products WHERE sku='SKU-VB-22')
);
...
WITH new_orders AS (
  ...
  SELECT
    ...
    v.totalprice
  FROM (VALUES
    -- Order 1: 1x SKU-ACME-24 + 2x SKU-VB-22
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-ACME-24') * 1
    + (SELECT price FROM myschema.products WHERE sku='SKU-VB-22')   * 2
    ),

    -- Order 2: 1x SKU-ACME-27
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-ACME-27') * 1
    ),

    -- Order 3: 1x SKU-VB-32
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-VB-32') * 1
    ),

    -- Order 4: 2x SKU-VB-22
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-VB-22') * 2
    ),

    -- Order 5: 2x SKU-ACME-24
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-ACME-24') * 2
    ),

    -- Order 6: 1x SKU-ACME-24 + 1x SKU-ACME-27
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-ACME-24') * 1
    + (SELECT price FROM myschema.products WHERE sku='SKU-ACME-27') * 1
    ),

    -- Order 7: 1x SKU-ACME-27 + 1x SKU-VB-22
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-ACME-27') * 1
    + (SELECT price FROM myschema.products WHERE sku='SKU-VB-22')   * 1
    ),

    -- Order 8: 1x SKU-VB-32 + 2x SKU-VB-22
    (
      (SELECT price FROM myschema.products WHERE sku='SKU-VB-32') * 1
    + (SELECT price FROM myschema.products WHERE sku='SKU-VB-22') * 2
    )
...
SELECT
  ...
  (SELECT product_id FROM myschema.products WHERE sku='SKU-ACME-27'),
  1,
  (SELECT price FROM myschema.products WHERE sku='SKU-ACME-27')

UNION ALL
-- Order 3 items
SELECT
  (SELECT order_id FROM orders_numbered WHERE rn=3),
  (SELECT product_id FROM myschema.products WHERE sku='SKU-VB-32'),
  1,
  (SELECT price FROM myschema.products WHERE sku='SKU-VB-32')
